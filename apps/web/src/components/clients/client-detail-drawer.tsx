...
interface ClientDetail {
  ...
  // (ClientsService.getClientDetail), не фронт по роли, чтобы не полагаться на клиентскую
  // проверку для чувствительных данных.
...
// Полное содержимое карточки клиента — вынесено из ClientDetailDrawer (запрос пользователя
// 2026-07-30, страница пересечения аудиторий: "при открытии записи покажет уже расширенную
// информацию этого клиента в двух каналах уже со всеми данными что есть в карточке клиента из
// списка клиентов, данные вплоть до данных рекламы") — нужно показывать ДВЕ такие карточки рядом
// (по одной на проект), а Drawer — выдвижная панель, для двух одновременно не подходит. Логика
// (запросы/мутации) не изменилась ни на строку, только шапка стала обычным div вместо
// DrawerHeader/DrawerTitle (те завязаны на контекст Drawer/vaul, вне него не гарантированно
// безопасны) — ClientDetailDrawer ниже оборачивает этот же компонент в Drawer для всех
// остальных мест приложения без единого изменения их поведения.
//
// readOnly (запрос пользователя 2026-07-30, тот же день: "внутри пересечения аудиторий убери
// функционал удаления клиента, редактирования, добавления покупок") — прячет форму "+ Добавить
// покупку" и кнопку "Удалить (GDPR)". По умолчанию false — обычный ClientDetailDrawer (страница
// клиентов, страница проекта) сохраняет все действия как раньше, флаг включает только страница
// пересечения аудиторий (сравнение read-only, менять/удалять данные оттуда не должно быть
// поводов — это просмотр, а не форма управления).
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        <section className="space-y-1.5">
          ...
          {/* user_id — обязательно для Telegram (запрос пользователя 2026-07-30: "если это
              телеграм то обязательно и user_id так как и по нему будут фильтровать") — это
              единственное реально стабильное поле для сопоставления клиента (username/имя
              меняются, tgUserId — никогда), то же поле, по которому строится пересечение
              аудиторий. Копируемый mono-текст, а не просто строка, для удобства вставки в
              поиск. */}
          {client.channelType === 'TELEGRAM' && client.tgUserId && (
            <div className="text-sm text-muted-foreground font-mono select-all">user_id: {client.tgUserId}</div>
          )}
...
export function ClientDetailDrawer({ projectId, clientId, onClose }: ClientDetailDrawerProps) {
  return (
    <Drawer open={!!clientId} onOpenChange={(open) => !open && onClose()} direction="right">
      <DrawerContent>{clientId && <ClientDetailContent projectId={projectId} clientId={clientId} onClose={onClose} />}</DrawerContent>
    </Drawer>
  );
}
