  ...
  },
  {
    ...
    body: (
      <>
        ...
        <Sub>Менеджеры в боте</Sub>
        <P>
          Если личный аккаунт не подключён, диалоги и покупки можно регистрировать прямо в Telegram. Добавьте
          username менеджеров в настройках бота (вкладка «Бот», карточка «Менеджеры»). Менеджер присылает боту
          клиента — пересланное сообщение, <Code>@username</Code>, ссылку <Code>t.me/…</Code> или Telegram ID
          цифрами — и видит карточку клиента с двумя кнопками. «Диалог» записывает диалог сразу. «Покупка» просит
          сумму в долларах следующим сообщением (<Code>150</Code>, <Code>49.99</Code>) и регистрирует депозит так
          же, как форма «Добавить покупку»: с событием Purchase для пикселей и сценарием первого или повторного
          депозита. Если клиент скрывает себя при пересылке, пришлите его username или ID.
        </P>
      </>
    ),
  },
