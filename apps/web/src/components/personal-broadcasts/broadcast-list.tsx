...
// Извлечено из .../projects/[id]/personal-broadcasts/page.tsx (запрос пользователя 2026-08-14:
// "у оператора тоже должна быть вкладка для этих пушей... точно такая же страница") —
// параметризовано projectId/newHref вместо чтения id из useParams, чтобы ОДИН И ТОТ ЖЕ компонент
// рендерился и на исходном project-scoped роуте (id из URL), и на новой company-wide странице
// /my-personal-broadcasts для Operator (id — из выбора в пикере, роут не project-scoped). Тот же
// приём, что уже применён к PushComposer (initialProjectIds prop) для аналогичной ситуации.
export function PersonalBroadcastsList({ projectId, newHref }: { projectId: string; newHref: string }) {
