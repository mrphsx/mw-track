...
// отмечен как "БЕЗ БАЕРА". Экспортирован (запрос пользователя 2026-09-03: ссылка для проекта
// типа "Обычный сайт", см. get-website-link-dialog.tsx) — та же логика, второй копии не нужно.
export const ELEVATED_ROLES = ['OWNER', 'ADMIN', 'SUPER_ADMIN'];

export function resolveLinkBuyerId(
  user: { role: string; buyerShortCode?: string | null; id: string } | null | undefined,
): string | null {
  if (!user) return null;
  ...
  return isElevated ? null : (user.buyerShortCode || user.id);
}
...
export const ALL_PIXELS = '__all__';
...
// Пиксель + готовая ссылка с копированием — общая часть для GetLinkDialog (ссылка лендинга) и
// GetWebsiteLinkDialog (ссылка проекта типа "Обычный сайт", запрос пользователя 2026-09-03) —
// единственное, что у них отличается, это откуда берётся destination-часть ссылки
// (домен+путь лендинга vs. Channel.websiteUrl), сама форма выбора пикселя и блок с готовой
// ссылкой идентичны один в один.
export function PixelLinkFields({
  pixels,
  pixelSelection,
  onPixelChange,
  link,
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-1.5">
        ...
        <Select value={pixelSelection} onValueChange={(v) => v && onPixelChange(v)}>
